...
export interface SafetyData {
  ...
  emergency: {
    ...
    police: { distance: string };
    embassy: { distance: string };
  };
...
export const authRegister = (
  ...
  api.post<ApiResp<AuthResp>>("/auth/register", {
    email,
    password,
    firstName,
    lastName,
...
export const getGems = (token: string, city?: string) =>
  api.get<ApiResp<Gem[]>>(`/search/services/gems${city ? `?city=${city}` : ''}`, token);

export const getAlerts = (token: string, city?: string) =>
  api.get<ApiResp<Alert[]>>(`/search/services/alerts${city ? `?city=${city}` : ''}`, token);

export const getTransit = (token: string, city?: string) =>
  api.get<ApiResp<TransitData>>(`/search/transit${city ? `?city=${city}` : ''}`, token);

export const getSafety = (token: string, city?: string) =>
  api.get<ApiResp<SafetyData>>(`/search/safety${city ? `?city=${city}` : ''}`, token);
